...
function open_popup(buttonelement, menuname, e)
{
	...
	if (!is_ie)
	{
		var elmtop = buttonelement.offsetHeight;
		// map is a new javascript function available in Gecko 1.8+ in which this bug wasfixed
		if (!popupcontrols.map)
		{
			var elmleft = getOffsetLeft(buttonelement);
			elmtop += getOffsetTop(buttonelement);
...
{
	alert("This editor is running with vbulletin_wysiwyg.js, CVS version " + String("$Revision: 1.72.2.1 $").replace(/^\$(Revision): ([0-9\.]+) \$$/, '$2'));
}

/*======================================================================*\
|| ####################################################################
|| # Downloaded: 09:31, Thu Nov 3rd 2005
|| # CVS: $RCSfile: vbulletin_wysiwyg.js,v $ - $Revision: 1.72.2.1 $
|| ####################################################################
\*======================================================================*/