...
define('CVS_REVISION', '$RCSfile: user.php,v $ - $Revision: 1.256.2.12 $');
...
if ($_REQUEST['do'] == 'find')
{
	...
	globalize($_REQUEST, array('orderby' => STR, 'limitstart' => INT, 'limitnumber' => INT, 'direction' => STR));
...
if ($_REQUEST['do'] == 'emailpassword')
{
	...
	construct_hidden_code('url', "$admincpdir/user.php?do=find&user[email]=" . urlencode($email));
	print_table_header($vbphrase['email_password_reminder_to_user']);
	print_description_row(construct_phrase($vbphrase['click_the_button_to_send_password_reminder_to_x'], "<i>" . htmlspecialchars($email) . "</i>"));
...
if ($_POST['do'] == 'update')
{
	...
	require_once('./includes/functions_misc.php');
	$temparray = array_merge($olduserinfo , $options);
	$user['options'] = convert_array_to_bits($temparray, $_USEROPTIONS);
	...
	{
		$user['lastvisit'] = $user['lastactivity'];
...
|| # CVS: $RCSfile: user.php,v $ - $Revision: 1.256.2.12 $
|| ####################################################################
\*======================================================================*/
?>