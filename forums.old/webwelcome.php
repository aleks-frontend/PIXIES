...
if ($showpminfo == "yes")
{
	...
	if ($enablepms==1 and $permissions['canusepm'] and $bbuserinfo['receivepm']) {
	  ...
	  eval("\$pminfo = \"".gettemplate('webwelcome_pminfo')."\";");
...
if ($showwhosonline == "yes")
{
	  ...
	  if ($loggedin=$DB_site->fetch_array($loggedins)) {
		...
		$username = "<b>$loggedin[username]</b>";
	      ...
	      {
	      eval("\$activeusers = \"".gettemplate('webwelcome_whosonline_usernames')."\";");
	      }
	    }
...
if ($bbuserinfo['userid']!=0) {
  ...
  eval("\$newposts = \"".gettemplate('webwelcome_newposts')."\";");

} else {
  $welcometext = "";
  eval("\$newposts = \"".gettemplate('webwelcome_todayposts')."\";");
...
eval("dooutput(\"".gettemplate('webwelcome')."\");");
