...
define('CVS_REVISION', '$RCSfile: image.php,v $ - $Revision: 1.74.2.4 $');
...
if ($_POST['do'] == 'insert')
{
	...
	if (!$path OR ($table == 'smilie' AND !$smilietext))
	{
		print_stop_message('please_complete_required_fields');
	}
	...
	$DB_site->query("
		INSERT INTO " . TABLE_PREFIX . "$table (
			...
			NULL,'" . addslashes($title) . "'," . iif($table == 'avatar', intval($minimumposts) . ',', '') . iif($table == 'smilie', "'" . addslashes($smilietext) . "',", '') . "'" . addslashes($path) . "'," . intval($imagecategoryid) . ',' . intval($displayorder) . "
		)
	");
...
if ($_REQUEST['do'] == 'add')
{
	...
	print_input_row($vbphrase["{$itemtype}_file_path"], 'path');
...
if ($_REQUEST['do'] == 'edit')
{
	...
	print_input_row($vbphrase["{$itemtype}_file_path"], 'path', $item["$itempath"]);
	print_select_row($vbphrase["{$itemtype}_category"], 'imagecategoryid', fetch_image_categories_array($catid), $item['imagecategoryid']);
...
|| # CVS: $RCSfile: image.php,v $ - $Revision: 1.74.2.4 $
|| ####################################################################
\*======================================================================*/
?>