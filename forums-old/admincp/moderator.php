...
define('CVS_REVISION', '$RCSfile: moderator.php,v $ - $Revision: 1.69 $');
...
if ($_REQUEST['do'] == 'remove')
{
	...
	print_delete_confirmation('moderator', $moderatorid, 'moderator', 'kill', 'moderator');
}
...
if ($_REQUEST['do'] == "showlist")
{
	...
	while ($moderator = $DB_site->fetch_array($moderators))
	{
		...
		echo "\t\t\t<li><a href=\"../forumdisplay.php?s=$session[sessionhash]&forumid=$moderator[forumid]\" target=\"_blank\">$moderator[title]</a>\n".
			...
			construct_link_code($vbphrase['delete'], "moderator.php?s=$session[sessionhash]&do=remove&moderatorid=$moderator[moderatorid]") . ")</span>\n".
			"\t\t\t</li><br />\n";
	}
	echo "\t\t</ul>\n\t\t</ul>\n\t</li>\n\t</ul>\n";
	echo "</div>\n";
...
|| # CVS: $RCSfile: moderator.php,v $ - $Revision: 1.69 $
|| ####################################################################
\*======================================================================*/
?>