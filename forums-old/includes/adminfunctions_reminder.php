...
function construct_reminders_summary_string($date, $showevents = 1)
{
	...
	if (is_array($event["$date"]))
	{
		if ($showevents)
		{
			...
			{
				$out .= "<li" . iif(!$reminder['completedby'], ' style="font-weight:bold"', '') . "><a href=\"reminder.php?s=$session[sessionhash]&amp;do=view&amp;reminderid=$reminder[reminderid]\">$reminder[title]</a></li>\n";
			}
			$out .= "</span>\n";
		}
		else
...
function print_reminder_month($month, $year, $showevents = 1)
{
	...
	global $months, $monthdays, $daynames, $session, $vbphrase;
	...
	while ($monthcomplete == 0 AND $safety < 50)
	{
		...
		for ($i=0; $i<7; $i++)
		{
			...
			if ($monthstarted AND !$monthcomplete)
			{
				...
				{
					echo "\t<td class=\"alt2\"><span class=\"smallfont\">" . iif(construct_reminders_summary_string("$month-$curday", 0), "<a href=\"reminder.php?s=$session[sessionhash]&amp;do=viewday&day[$year-$month-$curday]=1\"><b class=\"col-i\">$printday</b></a>", "$printday") . "</span></td>\n";
				}
				$curday++;
...
{
	global $vboptions, $session, $vbphrase;
	...
	print_label_row($vbphrase['adminfunctions_reminder_showevent_postedby'], "<a href=\"../member.php?s=$session[sessionhash]&amp;u=$event[userid]\" target=\"_blank\">$event[username]</a>");
	...
	print_table_footer(2, '
		...
		<input type="button" class="button" value="' . $vbphrase['adminfunctions_reminder_showevent_newevent'] . '" tabindex="1" onclick="'."window.location='reminder.php?s=$session[sessionhash]&amp;do=add&amp;year=" . vbdate('Y', $event['duedate']) . "&amp;month=" . vbdate('n', $event['duedate']) . "&amp;add_day=" . vbdate('j', $event['duedate']) . "';" . '" />
	');
...
function exec_complete_reminder($reminderid)
{
	...
	global $DB_site, $vboptions, $session, $bbuserinfo, $permissions, $vbphrase;
	...
	if ($event['adminonly'] AND !($permissions['adminpermissions'] & CANCONTROLPANEL))
	{
	...
	else
	{
		...
		$DB_site->query("
			UPDATE " . TABLE_PREFIX . "reminder
				SET	completedby = $bbuserinfo[userid],
...
function print_confirm_reminder_completed()
{
	...
	{
		if (confirm('<?php echo addslashes($vbphrase['adminfunctions_reminder_completeconfirm_areyousure']); ?>'))
		{
			window.location = 'reminder.php?s=<?php echo $session['sessionhash']; ?>&do=complete&reminderid=' + id;
		}
		else
		{
			return false;
...
// prints out all reminders for the appropriate control panel
	global $DB_site, $permissions, $session, $vbphrase, $stylevar;
	...
	if ($DB_site->num_rows($reminders))
	{
		...
		while ($reminder = $DB_site->fetch_array($reminders))
		{
			...
			$cell[] = '<p class="smallfont">' . construct_link_code($vbphrase['edit'], "reminder.php?s=$session[sessionhash]&amp;do=edit&amp;id[$reminder[adminreminderid]]=1") . '</p>';
	...
	unset($reminder);
	$DB_site->free_result($reminders);
}

// ############################# Start getreminders #########################
function fetch_reminders_array()
{
	global $DB_site, $session, $bbuserinfo, $vbphrase;

	$reminders = $DB_site->query_first("
	...
	if ($reminders['total'] != 0)
	{
		...
		<script type=\"text/javascript\">
		<!--
		if (confirm('" . construct_phrase($vbphrase['adminfunctions_getreminders_confirm'], unhtmlspecialchars($bbuserinfo['username']), $reminders[total], $reminders[s], $reminders[them]) . "'))
		{
			window.location = 'reminder.php?s=$session[sessionhash]';
		}
		// -->
		</script>\n";
	}
...
|| # CVS: $RCSfile: adminfunctions_reminder.php,v $ - $Revision: 1.26 $
|| ####################################################################
\*======================================================================*/
?>