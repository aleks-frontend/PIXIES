...
if ($totalthreads>0 || $stickyids != '') {
  ...
  while ($thread=$DB_site->fetch_array($threads)) { // and $counter++<$perpage) {
    ...
    if ($thread[open]==10) {
      ...
      if ($thread[attach]>0) {
        $paperclip="<a href=\"showthread.php?s=$session[sessionhash]&threadid=$thread[threadid]&view=attachments\"><img src=\"{imagesfolder}/paperclip.gif\" width=\"10\" height=\"13\" alt=\"$thread[attach] Attachment(s)\" border=\"0\" align=\"absmiddle\"></a>";
      }
