...
echo("<td style=\"font-family:$f; font-size:$hfs; color:$tc;\"><b><nobr>Sex News</nobr></b></td>\n");
...
while ($threads = mysql_fetch_array($tr)) {	
	...
	echo("<a class=small href=\"$url/showthread.php?threadid=$threads[threadid]\"  title=\"$threads[title]\">$title</a></nobr></td>\n");
	// last poster column?
	if ($lastposter == "1") {
	...
	if ($showmessages == "1") {
		...
		echo("<tr bgcolor=\"$bc\"><td style=\"font-family:$f; font-size:$fs; color:$tc;\" align=\"right\" valign=\"top\"></td>\n");
